...
A common Reddit claim says Google shut Candy AI down. That mixes adult app-store removals with a full product kill. Candy kept the website and PWA route. Aggregate review scores near 3.7 out of 5 across a few hundred public ratings mostly flag token burn and billing confusion. Stolen-card style complaints are rare compared with other companion apps.`,
  ...
  howToLead: `I followed one path on desktop and phone. No install beyond a browser. These are the exact steps from empty account to Story Mode.`,

  howToFree: `Free access exists, but it is a demo. You can create an account, browse characters, and send a limited batch of messages. That is enough to judge writing tone and load speed. Deeper uncensored chat, reliable image output, voice, and Live Action sit behind paid plans and tokens. There is no separate week-long premium trial. No peer in this tier gives a forever-free full media product either. Sample free, then decide if the annual plan matches the faces you see.`,

  howWorks: `Each session ties a language model to an image model. The language side reads recent turns plus your personality settings and writes the next line. The image side uses a character seed so faces stay recognizable across stills. Paid text stays available. Photos, voice minutes, and video draw from the token wallet. Text stays cheap. Media is where money goes.
...
In practice you get a character chatbot with a photo studio attached. Gallery presets and custom builds feed the same loop: history shapes the next reply, and the seed shapes the next frame.`,
  ...
  costExtra: `How long you commit sets the headline rate. Longer plans cut the monthly number. Each paid tier includes 100 tokens per month for media, with packs for sale when the wallet hits zero. The table below is the 2026 pricing I checked for this write-up.
